...
def list_files(functional_dir, extension, output_file):
    ...
    if section_type == 'listings':
            ...
            for item in functional_unit_dir.iterdir():
                if not item.name == functional_unit_dir.name+'.'+extension:
                    if item.is_file() and item.name.endswith('.' + extension):
                        with open(posixpath.join(functional_unit_dir, item.name), 'r') as f:
                            # Escaped name of functional unit
                            item_name_escaped = item.name
                            i = 0
                            while i < len(item_name_escaped):
                                if item_name_escaped[i] == '_':
                                    item_name_escaped = item_name_escaped[:i] + '\\' + item_name_escaped[i:]
                                    i = i + 1
                                i = i + 1

                            output.write(item_name_escaped+'\n\\lstinputlisting['+style_text+']{'+rel_dir+'/'+functional_unit_dir.name+'/'+item.name+'}\n\n')
                    elif not item.is_file():
                        first = True
                        overall_first = False
                        for individual_block in item.iterdir():
                            # Escaped name of individual block
                            ind_name_escaped = individual_block.name
                            i = 0
        ...
        for functional_unit_dir in Path(functional_dir).iterdir():
            ...
            # Escape intro_text
            intro_text_escaped = intro_text
            i = 0
            while i < len(intro_text_escaped):
                if intro_text_escaped[i] == '_':
                    intro_text_escaped = intro_text_escaped[:i] + '\\' + intro_text_escaped[i:]
                    i = i + 1
                i = i + 1

            # Escape input_text
            input_text_escaped = input_text
            i = 0
            while i < len(input_text_escaped):
                if input_text_escaped[i] == '_':
                    input_text_escaped = input_text_escaped[:i] + '\\' + input_text_escaped[i:]
                    i = i + 1
                i = i + 1

            # Escape output_text
            output_text_escaped = output_text
            i = 0
            while i < len(output_text_escaped):
                if output_text_escaped[i] == '_':
                    output_text_escaped = output_text_escaped[:i] + '\\' + output_text_escaped[i:]
                    i = i + 1
                i = i + 1
            ...
            output.write(intro_to_write + '\n\\begin{itemize}\n\\item \\textbf{Inputs:  } ' + input_text_escaped + '\n\\item \\textbf{Outputs: } ' + output_text_escaped + '\n\\end{itemize}\n' + body_to_write + '\\clearpage\n\n')
